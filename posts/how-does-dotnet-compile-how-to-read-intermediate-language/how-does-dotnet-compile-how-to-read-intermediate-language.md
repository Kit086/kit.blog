...
在编写代码或读其他开发者的代码时，如果不知道这段代码做了什么，我会感觉心里有点没底。同样，在编写基于 dotnet 的代码时，如果不知道它是如何编译的，也会感觉写起来不够放心。所以今天搜集资料学习了一下它的编译过程。dotnet 平台的语言很多，本篇以 C# 举例。
...
2. 当运行 dotnet 程序时，操作系统调用公共语言运行时（Common Language Runtime，简称 CLR）。CLR 将 CIL 进行 JIT 编译，编译为运行改代码的平台的机器码（Native code）。这使得能够静态编译位 CLI 的语言可以在很多平台和编译器类型上运行。但是使用虚拟机和 JIT 编译器来运行代码会导致性能不够好。
...
到这里，C# 编译过程就讲完了，这里放一个外网的图来形象的描述这个过程：
