...
last_updated: 2023-3-5 19:30:00
...
首先我们验证一下 `IEnumetable<T>` 可能被多次迭代的事实：
...
0 到 4 这 5 个数字被输出了两遍。原因是 `Console.WriteLine($"Tasks Count: {tasks.Count()}");` 这行代码在 `tasks.Count()` 时又遍历了 `tasks` 一遍。如果 `IEnumerable<T>` 集合中元素的数量很大，会有性能问题；如果集合中有像示例代码中这种操作，甚至会导致代码行为与我们理想的行为大不相同。

我们可以把代码改写成这样子来轻易解决：`Task[] tasks = Enumerable.Range(0, 5).Select(i => Task.Run(() => Console.WriteLine(i))).ToArray();`，但何时使用 `IEnumerable<T>`，何时使用其它集合类型，是一个值得讨论的话题。
...
如果你只想知道 `IEnumerable, IQueryable, ICollection` 的区别的话，只看这一节就可以了。但如果你搞清楚什么时候该选什么，或者想用到 `IReadOnlyCollection<T>` 等，或者知道它们有什么独特的优势，有什么特别的，容易产生异常和 bug 的致命缺点的话，可以往下多看几节。

### `IEnumerable<T>`

`IEnumerable<T>` 是最常用的接口。它是顶级接口，极为灵活。它可以表示任何类型的集合，包括数组、列表、集合、字典、文件、网络流等等。 **实现它的类型可以用 `foreach` 遍历，在需要循环遍历并对集合中的元素执行操作时可以选择它。**另外它还可以使用 `yield` 语句返回值。相信各位 C# 老法师不需要我给出示例代码了。
...
**当涉及到数据库时，`IQueryable<T>` 是更加高效的选择，因为它将过滤操作委托给数据库本身，从而实现更加高效的操作。**

### `ICollection<T>`

它除了有 `IEnumerable<T>` 的特性之外，还支持**添加或删除集合中的元素**，使用 `Add()` 和 `Remove()` 等方法。
...
2. 当需要在数据源上执行 LINQ 操作以提升效率时，选择 `IQueryable<T>`，典型场景是使用 EF Core 等 ORM 时；
3. 其他情况选择 `IEumerable<T>`。
...
这就带出了一个 **“抽象泄露”的概念。本来我们用抽象就是为了不需要了解它的实现细节就使用它，但 `IQueryable` 这个抽象却依然需要我们了解它的细节，所以说它是抽象泄露。举个例子：**
...
因此，我认为不应该把 `IQueryable<T>` 作为返回返回值类型。因为返回了它，就相当于告诉我的这个函数的调用者可以随便使用 LINQ 表达式来操作我的 `IQueryable<T>`。如果调用者使用了类似 `Select(MapToPersonDto)` 这种语句，可能会出现异常。

那还该不该用 `IQueryable<T>`？当然该用，但应该在 `Repository` 内部用，不要泄露给外部。这里我默认大家都喜欢 `Repository` 模式，不讨论是否该用这种模式。示例代码：
...
这样我们还是以 `IEnumerable<T>` 作为了返回值类型，并没有把 `IQueryable<T>` 暴露出去。调用方也不需要知道 EF Core 对 `IQueryable` Provider 的实现细节。这就成为了一个很好的抽象。但在函数内部，我们还是可以使用 `IQueryable` 和它的特性。

### `IReadOnlyCollection<T>` 与 `IReadOnlyList<T>`

由前面的结论，我们发现，**返回值类型还有告诉调用方可以或不可以用返回值做什么的功能。**

我们依然以 `Repository` 层为例。我们应该通过 `Repository` 的方法的返回值类型来告诉调用方，该方法返回的集合能否被改变。 **调用方可能希望通过改变 `Repository` 返回的集合来增删改数据源的数据，或者重新利用集合里的数据来做别的事情**，例如复用 `GetAdults()` 方法返回的集合变量，再添加一个你自己构造的 Person 对象进去：
...
为了保证代码的可维护性，我一般不会希望调用方做这种操作。如果调用方通过改变 `Repository` 中的方法的返回值而改变了数据源（使用 EF Core 等 ORM 时确实有可能），我认为会使得代码变乱，可维护性变差，后续版本迭代的时候会非常难受。我也不希望调用方复用 `Repository` 中的方法的返回值集合来添加或删除集合中的元素供后续使用，这也会让代码可读性和可维护性变差。这样很容易造出不易迭代的软件产品。我希望用户用返回值来创建一个新的集合以实现这些操作。不必要的数据修改通常会导致一系列难以调试的问题，此时可以通过使返回集合类型不可变来避免。

因此，`T[]` 数组和 `List<T>`、`IList<T>` 等就不在我的选择范围内了，因为它们是可被改变的。这种情况下我会选择 `IReadOnlyCollection<T>` 或 `IReadOnlyList<T>` 等不可变集合类型。

而且他们的功能比 `IEnumerable<T>` 更多，`IReadOnlyCollection<T>` 直接有 `Count` 属性，可以避免文章开头说过的多次遍历问题：
...
而 `IReadOnlyList<T>` 甚至还有用索引访问元素的功能，如 `tasks[0]` 这种。具体详情可以看官方文档，选择你需要的返回值类型。
